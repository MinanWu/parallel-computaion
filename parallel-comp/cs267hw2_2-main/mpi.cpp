...
void init_simulation(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
	...
	} while(panels_per_row < 1 || panels_per_col < 1 || blocks_per_panel_row < 3 || blocks_per_panel_col < 3); 
	...
	if (rank < rank_size) {
        ...
        panel_buffer_size = std::ceil(num_parts / num_panels) * 16;
        block_buffer_size = std::ceil(num_parts / num_blocks) * 16;
        ...
        for (int block_index = 0; block_index < num_blocks_per_panel; block_index++) {
            for (int i = blocks[block_index].size() - 1; i >= 0; i--) {
                ...
                if (panel_index_x == panel_x && panel_index_y == panel_y) {
                    ...
                    if (new_rank == rank - panels_per_row - 1 && panel_x > 0 && panel_y > 0) {
                        particle_t particle = blocks[block_index][i];
                        send_parts_buffer[0].push_back(particle);
                        blocks[block_index].erase(blocks[block_index].begin() + i);
                    } 
                    else if (new_rank == rank - panels_per_row + 1 && panel_x < (panels_per_row-1) && panel_y > 0) {			
                        particle_t particle = blocks[block_index][i];
                        send_parts_buffer[1].push_back(particle);
                        blocks[block_index].erase(blocks[block_index].begin() + i);
                    } 
                    else if (new_rank == rank + panels_per_row - 1 && panel_x > 0 && panel_y < (panels_per_col-1)) {	
                        particle_t particle = blocks[block_index][i];
                        send_parts_buffer[2].push_back(particle);
                        blocks[block_index].erase(blocks[block_index].begin() + i);
                    } 
                    else if (new_rank == rank + panels_per_row + 1 && panel_x < (panels_per_row-1) && panel_y < (panels_per_col-1)) {	
                        particle_t particle = blocks[block_index][i];
                        send_parts_buffer[3].push_back(particle);
                        blocks[block_index].erase(blocks[block_index].begin() + i);
                    } 
                    else if (new_rank == rank - panels_per_row && panel_y > 0) {
                        particle_t particle = blocks[block_index][i];
                        send_parts_buffer[4].push_back(particle);
                        blocks[block_index].erase(blocks[block_index].begin() + i);
                    } 
                    else if (new_rank == rank + panels_per_row && panel_y < (panels_per_col-1)) {
                        particle_t particle = blocks[block_index][i];
                        send_parts_buffer[5].push_back(particle);
                        blocks[block_index].erase(blocks[block_index].begin() + i);
                    } 
                    else if (new_rank == rank - 1 && panel_x > 0) {
                        particle_t particle = blocks[block_index][i];
                        send_parts_buffer[6].push_back(particle);
                        blocks[block_index].erase(blocks[block_index].begin() + i);
                    } 
                    else if (new_rank == rank + 1 && panel_x < (panels_per_row-1)) {			
                        particle_t particle = blocks[block_index][i];
