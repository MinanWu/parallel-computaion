...
static inline void assign_ghost_block_neighbors(int block_index) {
	...
	// Right most column
	if (panel_x < (panels_per_col - 1) && column_index == (blocks_per_panel_row - 1) && row_index > 0 && row_index < (blocks_per_panel_col - 1)) {
	...
	// Bottom row
	if (panel_y < (panels_per_row - 1) && row_index == (blocks_per_panel_col - 1) && column_index > 0 && column_index < (blocks_per_panel_row - 1)) {
...
void init_simulation(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
	...
	do{	
    		panels_per_col = std::ceil(sqrt(num_procs));
    		while(num_procs % panels_per_col != 0) {
        		panels_per_col += 1;
    		}
    		panels_per_row = num_procs / panels_per_col;

		rank_size = panels_per_col * panels_per_row;
		block_width = cutoff*3;
		...
		num_procs -= 1;
	}while(blocks_per_panel_row == 0 || blocks_per_panel_col == 0); 
...
void gather_for_save(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
	...
	if (rank < rank_size) {
		if (rank == 0) {
			int *global_cnt = new int[rank_size];
			...
			int *displacements = new int[rank_size];
			displacements[0] = 0;
			...
			//MPI_Gather(&size, 1, MPI_INT, NULL, 0, MPI_INT, 0, WORK_PROC);
