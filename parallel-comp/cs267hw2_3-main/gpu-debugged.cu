...
// debug code
// int* debug_count_cuda;
// int* debug_count;
// debug code


__global__ void set_ax_ay_gpu(particle_t* parts, int num_parts) {
...
void init_simulation(particle_t* parts, int num_parts, double size) {
	...
	cudaMalloc(((void**) &bin_start_indices), (num_bins * sizeof(int)));
	set_ax_ay_gpu<<<num_blocks, threads_per_block>>>(parts, num_parts);

	// debug code
	// cudaMalloc(((void**) &debug_count_cuda), (num_parts * sizeof(int)));
	// debug_count = new int[num_parts];
}
...
__global__ void compute_forces_gpu(particle_t* parts, int num_parts, double bin_width, int row_length, int* bin_counts, int* bin_start_indices, int* part_ids, int* part_bin_indices, int num_bins) {
	...
	if (bin_index < (num_bins - 1)) {
		...
		int bin_start_index = bin_start_indices[bin_index];
		for (int j = bin_start_index; ((j < num_parts) && (part_ids[j] != tid)); j += 1) {
			apply_force_symmetric_gpu(
				parts[tid],
				parts[part_ids[j]]
			);
...
__global__ void count_particles_gpu(particle_t* parts, int num_parts, double bin_width, int row_length, int* bin_counts, int* part_bin_indices) {
	...
	// debug code
	// atomicAdd(&(debug_count_cuda[tid]), tid);
...
void simulate_one_step(particle_t* parts, int num_parts, double size) {
	...
	// debug code
	// cudaMemcpy(debug_count, part_bin_indices, num_parts * sizeof(int), cudaMemcpyDeviceToHost);
	// cudaMemset(debug_count_cuda, 0, (num_parts * sizeof(int)));
	// int count = 0;
